...
输入描述：两行数据，第一行是同学的总数N ，第二行是N位同学的身高，以空格隔开
输出描述：最少需要几位同学出列
"""
...
def find_len(b):
    ...
    for i in range(1, len(nums)):#从初始的右边的人开始
        for j in range(i):#从初始（第0个）到第j个人的左边
            if nums[i] > nums[j] and dp[i] < dp[j] + 1: #若是第i个人的身高大于第j个人，而且自己的左边是j的话[dp[j] + 1]，得到的dp值大于自己本来得到的自己左边可以有多少人的计算[dp[i]]
...
while True:
    try:
        ...
        len_2 = find_len(student_tall[::-1]) #获得逆向从小到大（从大到小）len_2[::-1]代表每个学生的右边最多有几个人（包括自己）
        len_len_1 = len(len_1)
        len_3 = []
        for i in range(len_len_1):
            len_3.append(len_1[i] + len_2[::-1][i]) #两个相加就是每个学生的左右最多有几个人，结果包含两个自己
        print(student_num - max(len_3) + 1)#按照输出要求，要得到踢出去几个人，所以需要len_3 -1 就是每个学生左右加上自己最多有几个人，取最大的那个学生的值，再用总数减去就是踢出去最少的人
    except:
        break
